...
async def post_item_handler(request, body: ItemScheme):
    ...
    return HTTPResponse(
        f'{{"item": {response}}}', status=201, content_type="application/json"
    )
...
async def get_items_handler(request):
    ...
    return HTTPResponse(
        f'{{"items": {response}}}', status=200, content_type="application/json"
    )
...
async def update_handler(request, body, id: int):
    ...
    return HTTPResponse(
        f'{{"item": {response}}}', status=200, content_type="application/json"
    )
...
async def buy_item_handler(request, user, body: AccountInputScheme, id: int):
    ...
    if not account:
        raise SanicException("account number is incorrect", status_code=400)
