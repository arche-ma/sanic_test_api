...
async def create_user(user: UserScheme):
    if await User.get_or_none(username=user.username):
        raise exceptions.SanicException("user with this username already exists", 400)
...
async def get_user_by_uuid(uuid):
    ...
    if not user_link:
        raise exceptions.SanicException("activation link not found", status_code=404)
...
async def get_user_transactions(user_id):
    ...
    transactions = await Transaction.filter(account__user__id=user_id).prefetch_related(
        "account"
    )
    response = [
        {
